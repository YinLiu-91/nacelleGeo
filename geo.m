...
-0.802867383513 1.1877394636; 
-1.08243727599 1.12643678161; 
-1.18996415771 1.06896551724; 
-1.19713261649 1.01915708812; 
-1.15053763441 0.973180076628;
-1.0394265233 0.938697318008; 
-0.910394265233 0.931034482759; 
-0.720430107527 0.942528735632; 
-0.390681003584 0.984674329502; 
0 1; 
0.3 1; % dummy node
0.3 0.298850574713; % dummy node
0 0.298850574713; 
-0.0931899641577 0.27969348659; 
-0.204301075269 0.275862068966; 
...
for g = 1:length(nacelle) 
    if nacelle(1,g) < 0 && nacelle(2,g) <= 1.1 
        junk(:,g) = nacelle(:,g); 
    else 
        n(:,g) = nacelle(:,g); 
    end 
...
if length(x_info) >= length(y_info) 
    nacelle = n(:,x_info); 
else 
    nacelle = n(:,y_info); 
end
...
spinner = [spline(s2,x(15:21,1),0:0.1:max(s2));
spline(s2,y(15:21,1),0:0.1:max(s2))];
...
% Generate fan face %———————————————
fan_y = 0.298850574713:(1-0.298850574713)*1e-3:1; 
fan_x = zeros(1,length(fan_y));
% Redefining the geometry %———————————————% The fan radius is 1.2m
fan_x = fan_x'*1.2; fan_y = fan_y'*1.2;
liner = zeros(length(nacelle(1,end-34:end-2)),2);
nacelle = nacelle'*1.2; 
spinner = spinner'*1.2; 
nacelle(:,1) = nacelle(:,1)+0.123; 
spinner(:,1) = spinner(:,1)+0.123; 
liner(:,1) = nacelle(end-34:end-2,1); 
liner(:,2) = nacelle(end-34:end-2,2);

% plot generic inlet geometry %———————————————–
figure
% plot nacelle
plot(nacelle(:,1),nacelle(:,2),'Linewidth',1.5); 
hold on;
% plot liner position
